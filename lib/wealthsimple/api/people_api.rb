...
module Wealthsimple
  class PeopleApi
    ...
    # @return [Person]
    def create_person(person, opts = {})
      data, _status_code, _headers = create_person_with_http_info(person, opts)
      return data
    end
    ...
    # @param person 
    # @param [Hash] opts the optional parameters
    # @return [Array<(Person, Fixnum, Hash)>] Person data, response status code and response headers
    def create_person_with_http_info(person, opts = {})
      ...
      if @api_client.config.client_side_validation && person.nil?
        fail ArgumentError, "Missing the required parameter 'person' when calling PeopleApi.create_person"
      end
      # resource path
      ...
      # http body (model)
      post_body = @api_client.object_to_http_body(person)
      auth_names = ['Bearer']
    ...
    def get_person_with_http_info(person_id, opts = {})
      ...
      # resource path
      local_var_path = "/people/{person_id}".sub('{' + 'person_id' + '}', person_id.to_s)
    ...
    # @option opts [Array<String>] :person_ids Limits the results only to those with a &#x60;person_id&#x60; in the list.
    ...
    # @option opts [String] :updated_after Limits the results returned to only those that were updated after this date.
    # @option opts [String] :updated_before Limits the results returned to only those that were updated before this date.
    # @return [Array<(PeoplePaginated, Fixnum, Hash)>] PeoplePaginated data, response status code and response headers
    def list_people_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: PeopleApi.list_people ..."
      end
      if @api_client.config.client_side_validation && !opts[:'offset'].nil? && opts[:'offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"offset"]" when calling PeopleApi.list_people, must be greater than or equal to 0.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 250
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling PeopleApi.list_people, must be smaller than or equal to 250.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling PeopleApi.list_people, must be greater than or equal to 1.'
      end
      ...
      query_params[:'person_ids'] = @api_client.build_collection_param(opts[:'person_ids'], :csv) if !opts[:'person_ids'].nil?
    ...
    # @param [Hash] opts the optional parameters
    # @return [Person]
    def update_person(person_id, person, opts = {})
      data, _status_code, _headers = update_person_with_http_info(person_id, person, opts)
      return data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @return [Array<(Person, Fixnum, Hash)>] Person data, response status code and response headers
    def update_person_with_http_info(person_id, person, opts = {})
      if @api_client.config.debugging
      ...
      # verify the required parameter 'person' is set
      if @api_client.config.client_side_validation && person.nil?
        fail ArgumentError, "Missing the required parameter 'person' when calling PeopleApi.update_person"
      end
      # resource path
      local_var_path = "/people/{person_id}"
