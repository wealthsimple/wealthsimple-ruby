  ...
  class AccountsApi
    ...
    def create_account_with_http_info(account, opts = {})
      ...
      if @api_client.config.client_side_validation && account.nil?
        fail ArgumentError, "Missing the required parameter 'account' when calling AccountsApi.create_account"
      end
      # resource path
    ...
    # @option opts [Array<String>] :account_types Limit the results to certain account types
    ...
    # @option opts [Array<String>] :account_types Limit the results to certain account types
    # @return [Array<(AccountsPaginated, Fixnum, Hash)>] AccountsPaginated data, response status code and response headers
    def list_accounts_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: AccountsApi.list_accounts ..."
      end
      if @api_client.config.client_side_validation && !opts[:'offset'].nil? && opts[:'offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"offset"]" when calling AccountsApi.list_accounts, must be greater than or equal to 0.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 250
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling AccountsApi.list_accounts, must be smaller than or equal to 250.'
      end
      ...
      if @api_client.config.client_side_validation && opts[:'account_types'] && !opts[:'account_types'].all?{|item| ['ca_tfsa', 'ca_rrsp', 'ca_non_registered', 'us_individual', 'us_roth_ira', 'us_traditional_ira', 'gb_individual', 'gb_isa'].include?(item)}
        fail ArgumentError, 'invalid value for "account_types", must include one of ca_tfsa, ca_rrsp, ca_non_registered, us_individual, us_roth_ira, us_traditional_ira, gb_individual, gb_isa'
      end
      # resource path
      local_var_path = "/accounts"
      ...
      query_params[:'account_types'] = @api_client.build_collection_param(opts[:'account_types'], :csv) if !opts[:'account_types'].nil?
