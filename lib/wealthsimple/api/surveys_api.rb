...
module Wealthsimple
  class SurveysApi
    ...
    # 
    # Get a Survey entity
    # @param survey_id The &#x60;id&#x60; of the Survey entity.
    ...
    # @return [Survey]
    def get_survey(survey_id, opts = {})
      data, _status_code, _headers = get_survey_with_http_info(survey_id, opts)
      return data
    end
    ...
    def get_survey_with_http_info(survey_id, opts = {})
      ...
      if @api_client.config.client_side_validation && survey_id.nil?
        fail ArgumentError, "Missing the required parameter 'survey_id' when calling SurveysApi.get_survey"
      end
      if @api_client.config.client_side_validation && !opts[:'offset'].nil? && opts[:'offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"offset"]" when calling SurveysApi.get_survey, must be greater than or equal to 0.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 250
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling SurveysApi.get_survey, must be smaller than or equal to 250.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling SurveysApi.get_survey, must be greater than or equal to 1.'
      end
      ...
      # resource path
      local_var_path = "/surveys/{survey_id}".sub('{' + 'survey_id' + '}', survey_id.to_s)
    ...
    # @option opts [String] :sort_by Attribute to sort results by.
    # @option opts [String] :sort_order The sort direction of the results.
    # @return [Array<(SurveysPaginated, Fixnum, Hash)>] SurveysPaginated data, response status code and response headers
    def list_surveys_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: SurveysApi.list_surveys ..."
      end
      if @api_client.config.client_side_validation && !opts[:'offset'].nil? && opts[:'offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"offset"]" when calling SurveysApi.list_surveys, must be greater than or equal to 0.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 250
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling SurveysApi.list_surveys, must be smaller than or equal to 250.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling SurveysApi.list_surveys, must be greater than or equal to 1.'
      end
